...
describe("vote.js", function () {
  ...
  describe("#createVote", function () {
    var createVote = vote.createVote,
      vt = null,
      publicKey = ripajs.crypto.getKeys("secret").publicKey,
    ...
    it("should create vote from ecpair", function () {
      ...
      var secondSecretKey = ripajs.ECPair.fromSeed("second secret");
    ...
    it("should create vote from wif", function () {
      var secretKey = ripajs.ECPair.fromWIF("SB3iDxYmKgjkhfDZSKgLaBrp3Ynzd3yd3ZZF2ujVBK7vLpv6hWKK", NETWORKS.ripa);
      secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");
    ...
    describe("returned vote", function () {
      ...
      it("should have recipientId string equal to sender", function () {
        (vt).should.have.property("recipientId").and.be.type("string").and.equal(ripajs.crypto.getAddress(publicKey))
      });
      ...
      it("should be signed correctly", function () {
        var result = ripajs.crypto.verify(vt);
        (result).should.be.ok;
      });

      it("should be second signed correctly", function () {
        var result = ripajs.crypto.verifySecondSignature(vt, ripajs.crypto.getKeys("second secret").publicKey);
        (result).should.be.ok;
      });
